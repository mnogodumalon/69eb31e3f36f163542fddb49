...
import type { Mitarbeiter, Schichtdefinition, Schichtplanung, CreateMitarbeiter, CreateSchichtdefinition, CreateSchichtplanung } from '@/types/app';
...
export class LivingAppsService {
  ...
  // --- SCHICHTDEFINITION ---
  static async getSchichtdefinition(): Promise<Schichtdefinition[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.SCHICHTDEFINITION}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Schichtdefinition[];
    return enrichLookupFields(records, 'schichtdefinition');
  }
  static async getSchichtdefinitionEntry(id: string): Promise<Schichtdefinition | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.SCHICHTDEFINITION}/records/${id}`);
    const record = { record_id: data.id, ...data } as Schichtdefinition;
    return enrichLookupFields([record], 'schichtdefinition')[0];
  }
  static async createSchichtdefinitionEntry(fields: CreateSchichtdefinition) {
    return callApi('POST', `/apps/${APP_IDS.SCHICHTDEFINITION}/records`, { fields: cleanFieldsForApi(fields as any, 'schichtdefinition') });
  }
  static async updateSchichtdefinitionEntry(id: string, fields: Partial<CreateSchichtdefinition>) {
    return callApi('PATCH', `/apps/${APP_IDS.SCHICHTDEFINITION}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'schichtdefinition') });
  }
  static async deleteSchichtdefinitionEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.SCHICHTDEFINITION}/records/${id}`);
  }
