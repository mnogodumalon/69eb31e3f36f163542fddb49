...
import { SchichtdefinitionDialog } from '@/components/dialogs/SchichtdefinitionDialog';
import { SchichtdefinitionViewDialog } from '@/components/dialogs/SchichtdefinitionViewDialog';
...
const SCHICHTDEFINITION_FIELDS = [
...
const SCHICHTPLANUNG_FIELDS = [
  ...
  { key: 'schicht_ref', label: 'Schicht', type: 'applookup/select', targetEntity: 'schichtdefinition', targetAppId: 'SCHICHTDEFINITION', displayField: 'schichtname' },
  { key: 'arbeitsbereich', label: 'Arbeitsbereich / Station', type: 'string/text' },
...
const ENTITY_TABS = [
  ...
  { key: 'schichtdefinition', label: 'Schichtdefinition', pascal: 'Schichtdefinition' },
...
export default function AdminPage() {
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'schichtdefinition': return (data as any).schichtdefinition as Schichtdefinition[] ?? [];
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'schichtplanung':
        ...
        lists.schichtdefinitionList = (data as any).schichtdefinition ?? [];
        break;
    }
    return lists;
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'schichtplanung' && fieldKey === 'schicht_ref') {
      const match = (lists.schichtdefinitionList ?? []).find((r: any) => r.record_id === id);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'schichtdefinition': return SCHICHTDEFINITION_FIELDS;
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'schichtdefinition': return {
        create: (fields: any) => LivingAppsService.createSchichtdefinitionEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateSchichtdefinitionEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteSchichtdefinitionEntry(id),
      };
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'schichtdefinition' || dialogState?.entity === 'schichtdefinition') && (
        <SchichtdefinitionDialog
          open={createEntity === 'schichtdefinition' || dialogState?.entity === 'schichtdefinition'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'schichtdefinition' ? handleUpdate : (fields: any) => handleCreate('schichtdefinition', fields)}
          defaultValues={dialogState?.entity === 'schichtdefinition' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Schichtdefinition']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Schichtdefinition']}
        />
      )}
      {(createEntity === 'schichtplanung' || dialogState?.entity === 'schichtplanung') && (
        <SchichtplanungDialog
          ...
          schichtdefinitionList={(data as any).schichtdefinition ?? []}
      ...
      {viewState?.entity === 'schichtdefinition' && (
        <SchichtdefinitionViewDialog
          open={viewState?.entity === 'schichtdefinition'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'schichtdefinition', record: r }); }}
        />
      )}
