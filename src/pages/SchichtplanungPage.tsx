...
export default function SchichtplanungPage() {
  ...
  const [schichtdefinitionList, setSchichtdefinitionList] = useState<Schichtdefinition[]>([]);
  ...
  async function loadData() {
    setLoading(true);
    try {
      const [mainData, mitarbeiterData, schichtdefinitionData] = await Promise.all([
      ...
      setSchichtdefinitionList(schichtdefinitionData);
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    ...
    >
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; setViewingRecord(record); }}>
                ...
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{getSchichtdefinitionDisplayName(record.fields.schicht_ref)}</span></TableCell>
      ...
      <SchichtplanungDialog
        ...
        schichtdefinitionList={schichtdefinitionList}
